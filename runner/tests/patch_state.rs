...
use near_primitives::borsh::{self, BorshSchema, BorshSerialize, BorshDeserialize};
...
async fn view_status_state() -> (AccountId, StatusMessage) {
    let (contract_id, signer) = runner::dev_deploy(Path::new(STATUS_MSG_WASM_FILEPATH)).await.unwrap();
...
async fn test_view_state() {
    ...
    assert_eq!(status_msg, StatusMessage {
        records: vec![Record {
...
async fn test_patch_state() {
    ...
    let _outcome = runner::patch_state(
        contract_id.clone(), "STATE".to_string(), status_msg).await.unwrap();
